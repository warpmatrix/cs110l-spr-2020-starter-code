...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        let fd_path = fs::read_link(format!("/proc/{}/fd/{}", pid, fd)).ok()?;
        let fd_path_str = fd_path.to_str()?;
        let fd_name = OpenFile::path_to_name(fd_path_str);
        ...
        let openfile = OpenFile::new(fd_name, cursor, access_mode);
        Some(openfile)
    }
